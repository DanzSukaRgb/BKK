        ...
        <div class="col-md-8">
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <p><i class="fas fa-briefcase me-2"></i> <strong>Posisi:</strong> {{ $lowongan->posisi }}</p>
                            <p><i class="fas fa-clock me-2"></i> <strong>Jenis:</strong> {{ $lowongan->jenis_pekerjaan }}</p>
                            <p><i class="fas fa-graduation-cap me-2"></i> <strong>Pendidikan:</strong> {{ $lowongan->pendidikan }}</p>
                        </div>
                        <div class="col-md-6">
                            <p><i class="fas fa-money-bill-wave me-2"></i> <strong>Gaji:</strong> {{ $lowongan->gaji ?? 'Negosiasi' }}</p>
                            <p><i class="fas fa-calendar-alt me-2"></i> <strong>Buka:</strong> {{ $lowongan->tanggal_buka->format('d M Y') }}</p>
                            <p><i class="fas fa-calendar-times me-2"></i> <strong>Tutup:</strong> {{ $lowongan->tanggal_tutup->format('d M Y') }}</p>
                        </div>
                    </div>
                            ...
                            @if($lowongan->status == 'Aktif' && $lowongan->tanggal_tutup >= now())
                                ...
                                @else
                                    <form action="{{ route('lamaran.store', ['id' => $lowongan->id]) }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="mb-3">
                                            <label for="dokumen" class="form-label">Unggah Dokumen Pendukung (CV, Portofolio, dll)</label>
                                            <input type="file" class="form-control @error('dokumen') is-invalid @enderror"
                                                id="dokumen" name="dokumen" accept=".pdf,.doc,.docx" required>
                                            @error('dokumen')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="text-center mb-3">
                        <img src="{{ $lowongan->perusahaan->logo }}" alt="{{ $lowongan->perusahaan->nama }}" class="img-fluid rounded" style="max-height: 100px;">
                    </div>
                    <h5>{{ $lowongan->perusahaan->nama }}</h5>
                    <p class="text-muted"><i class="fas fa-industry me-2"></i> {{ $lowongan->perusahaan->industri }}</p>
                    ...
                    <p><i class="fas fa-globe me-2"></i> <a href="{{ $lowongan->perusahaan->website }}" target="_blank">{{ $lowongan->perusahaan->website }}</a></p>
                    @endif
                    <a href="{{ route('perusahaan.show.public', $lowongan->perusahaan->id) }}" class="btn btn-outline-primary btn-sm mt-2">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3 pb-3 border-bottom">
                        <h6><a href="{{ route('lowongan.show.public', $otherJob->slug) }}">{{ $otherJob->judul }}</a></h6>
                        <small class="text-muted">{{ $otherJob->posisi }} • {{ $otherJob->jenis_pekerjaan }}</small>
                    </div>
                    @endforeach
